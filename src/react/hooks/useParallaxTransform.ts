...
export const useParallaxTransform = ({
  ...
  const applyTransform = useCallback((transformValue: string) => {
  ...
  const manageTransition = useCallback((shouldApply: boolean) => {
    if (!imageRef.current || prefersReducedMotion) return;

    const transitionValue = shouldApply && delay > 0 
      ? `transform ${delay}s ${transition}` 
      : '';
